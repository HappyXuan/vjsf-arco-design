...
export default {
  ...
  setup(props) {
    ...
    const isRequired = (name) =>
      ...
      !!~props.schema.required.indexOf(name)
    ...
    const isDependOn = (name) => {
      ...
      if (isObject(props.schema.dependencies)) {
        curDependent = Object.entries(props.schema.dependencies).some(
          ([key, value]) => {
            // 是否和当前属性存在依赖关系
            const tempDependency = !!(
              Array.isArray(value) && ~value.indexOf(name)
            )
            ...
            return (
              tempDependency &&
              getPathVal(props.rootFormData, props.curNodePath)[key] !==
                undefined
            )
          }
        )
            ...
            ...(props.needValidFieldGroup
              ? [
                  h(Widget, {
                    ...
                    schema: Object.entries(props.schema).reduce(
                      (preVal, [key, value]) => {
                        if (
