  ...
  setup(props, { emit, attrs }) {
    ...
    const fixItemsFormData = () => {
      ...
      if (isNoArray || props.itemsFormData.length < props.schema.items.length) {
        // 这里需要补齐默认数据，计算出需要的数据
        const curSchemaState = getDefaultFormState(
          props.schema,
          undefined,
          props.rootSchema
        )
    ...
    return () => {
      ...
      const { schema, uiSchema, errorSchema, curNodePath, globalOptions } =
        props
      ...
      const cutOfArr = cutOff(
        props.itemsFormData,
        props.schema.items.length - 1
      )
      ...
      const trueAddable =
        (addable === undefined ? true : addable) &&
        allowAdditionalItems(props.schema)
