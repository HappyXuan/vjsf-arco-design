...
export default {
  ...
  setup(props, { emit }) {
    ...
    const widgetValue = computed({
      get() {
        if (props.isFormData)
          return getPathVal(props.rootFormData, props.curNodePath)
      ...
      set(value) {
        // 大多组件删除为空值会重置为null。
        const trueValue =
          value === '' || value === null ? props.emptyValue : value
        if (props.isFormData) {
    ...
    return () => {
      ...
      const miniDesModel =
        isMiniDes ?? props.globalOptions.HELPERS.isMiniDes(props.formProps)
      ...
      return h(
        ...
        },
        {
          ...
          help: (slotProps) =>
            !miniDesModel && descriptionVNode ? h(descriptionVNode) : [],
                      ...
                      ...(otherAttrs
                        ? (() =>
                            Object.keys(otherAttrs).reduce((pre, k) => {
                              ...
                              // 保证ui配置同名方法 ui方法先执行
                              ;[props.widgetAttrs[k], props.uiProps[k]].forEach(
                                (uiConfFn) => {
                                  if (
                                    uiConfFn &&
                                    typeof uiConfFn === 'function'
                                  ) {
                                    pre[k] = (...args) => {
                                      uiConfFn(...args)
                                      pre[k](...args)
